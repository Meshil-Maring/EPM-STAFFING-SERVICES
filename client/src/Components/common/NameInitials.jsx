import React, { useMemo } from "react";

function NameInitials({
  name,
  id,
  class_name = "h-12 bg-[#dd6b20] w-12 rounded-large font-semibold text-lg flex items-center justify-center",
}) {
  // Generate initials from name
  const initials = useMemo(() => {
    if (!name) return "?";
    const parts = name.trim().split(/\s+/);
    const first = parts[0]?.[0] || "";
    const second = parts[1]?.[0] || "";
    return (first + second).toUpperCase();
  }, [name]);

  return (
    <div className={`relative shadow-xl shrink-0 ${class_name || ""}`}>
      <div className=" overflow-hidden h-[70%] w-[70%] flex items-center justify-center text-text_white shadow-sm ">
        <span className={`${class_name} flex items-center justify-center`}>
          {initials}
        </span>
      </div>
      {id && (
        <span
          className="absolute -bottom-2 -right-2 p-1 rounded-small bg-whiter border border-lighter shadow-xs flex items-center justify-center text-[10px] font-bold text-secondary"
          aria-label={`Candidate number ${id}`}
        >
          #{id}
        </span>
      )}
    </div>
  );
}

export default NameInitials;
